import React, { useEffect, useState } from "react";
import { useLocation } from "react-router-dom"; // Use to access URL search params
import { db } from "../firebaseConfig"; // Assuming you're using Firebase
import { doc, getDoc } from "firebase/firestore";
import Navbar from "../components/Navbar";
import Footer from "../components/Footer";
import { format } from "date-fns";

const Blog = () => {
  const location = useLocation(); // Get the current location (URL)
  const [blogPost, setBlogPost] = useState(null); // State to store blog data
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Extract the 'id' from query parameters
    const queryParams = new URLSearchParams(location.search);
    const blogId = queryParams.get("id");

    if (blogId) {
      // Fetch the blog post from Firebase using the ID from URL
      const fetchBlogPost = async () => {
        try {
          const blogRef = doc(db, "blogs", blogId); // Access the blog document by ID
          const blogSnap = await getDoc(blogRef);

          if (blogSnap.exists()) {
            // If blog data exists, set it to the state
            setBlogPost(blogSnap.data());
          } else {
            console.log("No such blog!");
          }
        } catch (error) {
          console.error("Error fetching blog: ", error);
        } finally {
          setLoading(false); // Ensure loading is false even if an error occurs
        }
      };

      fetchBlogPost();
    }
  }, [location.search]); // Trigger the effect whenever the search params change

  if (loading) {
    return (
      <div className="flex justify-center items-center h-screen">
        <Navbar />
        <div className="animate-spin rounded-full h-12 w-12 border-t-4 border-primary-green border-solid"></div>
      </div>
    );
  }

  if (!blogPost) {
    return (
      <div className="text-center mt-10 text-gray-600">Blog not found.</div>
    );
  }

  return (
    <div className="mx-auto">
      <Navbar />
      <div
        aria-hidden="true"
        className="absolute inset-x-0 top-[-10rem] -z-10 transform-gpu overflow-hidden blur-3xl sm:top-[-20rem]"
      >
        <div
          style={{
            clipPath:
              "polygon(74.1% 44.1%, 100% 61.6%, 97.5% 26.9%, 85.5% 0.1%, 80.7% 2%, 72.5% 32.5%, 60.2% 62.4%, 52.4% 68.1%, 47.5% 58.3%, 45.2% 34.5%, 27.5% 76.7%, 0.1% 64.9%, 17.9% 100%, 27.6% 76.8%, 76.1% 97.7%, 74.1% 44.1%)",
          }}
          className="relative left-1/2 -z-10 aspect-1155/678 w-[36.125rem] max-w-none -translate-x-1/2 rotate-[30deg] bg-linear-to-tr from-[#646464] to-[#00ff81] opacity-30 sm:left-[calc(50%-40rem)] sm:w-[72.1875rem]"
        />
      </div>
      <div className="block w-full text-gray-900 bg-white rounded-t-none block-canvas">
        <div className="min-h-screen flex flex-col">
          {/* Cover Image */}
          <div className="w-full h-[200px] md:h-[300px] lg:h-[300px] overflow-hidden">
            <img
              src={blogPost.coverImageUrl}
              alt="Cover"
              className="w-full h-full object-cover"
            />
          </div>

          {/* Content Container */}
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            {/* Author Section */}
            <div className="flex items-center space-x-4">
              <img
                src={
                  blogPost.authorImage ||
                  "https://lirp.cdn-website.com/eda4ad32/dms3rep/multi/opt/41717CF9-288E-4F9C-BF86-9E35DA494F87-640w.jpg"
                }
                alt={blogPost.publisher}
                className="w-12 h-12 rounded-full object-cover"
              />
              <div>
                <p className="font-semibold text-gray-900">
                  {blogPost.publisher}
                </p>
                <p className="text-gray-500 text-xs">
                  {blogPost.designation} •{" "}
                  {blogPost.createdAt?.toDate
                    ? format(blogPost.createdAt.toDate(), "MMM dd, yyyy")
                    : "Unknown Date"}
                </p>
              </div>
            </div>

            {/* Blog Title */}
            <h1 className="text-3xl font-bold text-gray-900 mt-6 leading-tight">
              {blogPost.title}
            </h1>

            {/* Blog Content */}
            <div
              className="mt-4 text-gray-700 text-lg leading-relaxed whitespace-pre-line"
              dangerouslySetInnerHTML={{ __html: blogPost.content }}
            ></div>
          </div>
        </div>
      </div>
      <Footer />
    </div>
  );
};

export default Blog;
